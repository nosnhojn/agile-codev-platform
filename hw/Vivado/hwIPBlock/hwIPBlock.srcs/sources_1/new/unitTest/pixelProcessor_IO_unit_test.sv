...
//   #2 $display("%t: rdata:0x%0x raddr:%0x", $time, rdata, raddr);
//   #5 $display("%t: rdata:0x%0x raddr:%0x", $time, rdata, raddr);
 end
...
//my_qpram.gross = 1;
  ...
  task expectEgressPixel(bit [23:0] data, bit user = 1, bit[3:0] keep = 'hb, bit last = 0);
    nextSamplePoint();
  //$display("%t - EXPECT raddr:0x%0x rdata:0x%0x oTDATA:0x%0x", $time, raddr, rdata, oTDATA);
